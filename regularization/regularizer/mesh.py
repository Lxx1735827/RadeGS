    ...
    if iteration >= config["start_iter"]:
        ...
        if use_delaunay_downsampling:
            if reset_delaunay_samples:
                ...
                if downsample_gaussians_for_delaunay:
                    print(
                        f"[INFO] Downsampling Delaunay Gaussians from {n_gaussians_to_sample_from} to {n_max_gaussians_for_delaunay}.")
                    ...
                    print(
                        f"[INFO] Downsampled Delaunay Gaussians from {n_gaussians_to_sample_from} to {len(delaunay_xyz_idx)}.")
        ...
        if not reset_sdf_values:
            ...
            if n_voronoi_sdf != voronoi_points.shape[0]:
                print(
                    f"[WARNING] Delaunay SDFs ({n_voronoi_sdf}) and points ({voronoi_points.shape[0]}) count mismatch. Resetting SDFs.")
                reset_sdf_values = True
        ...
        if reset_sdf_values:
            with torch.no_grad():
                ...
                base_occupancy = compute_initial_sdf_with_binary_search(
                    ...
                    enforce_std=config["sdf_reset_linearization_enforce_std"] if config[
                                                                                     "n_binary_steps_to_reset_sdf"] > 0 else None,
                )  # Between -1 and 1
        ...
        if (
                ...
                or reset_occupancy_labels_for_new_delaunay_sites
        # If not fixing sites and downsampling, compute labels for sampled sites
        )
        ):
            ...
            print(
                f"[INFO] Points with label > 0.5: {torch.sum(voronoi_occupancy_labels > 0.5) / voronoi_occupancy_labels.numel()}")
        ...
        if config["use_depth_loss"]:
            ...
            if config["mesh_depth_loss_type"] == "log":
                mesh_depth_loss = torch.log(
                    1. + (mesh_depth - gaussians_depth).abs() / gaussians.spatial_lr_scale)  # (H, W)
        ...
        if config["enforce_occupied_centers"]:
            ...
            if mesh_state["surface_delaunay_xyz_idx"] is not None:
                gaussians_occupancy = gaussians.get_occupancy[
                    mesh_state["surface_delaunay_xyz_idx"]]  # (N_surface_gaussians, 9)
                gaussians_occupancy = gaussians_occupancy[:, -1]  # (N_surface_gaussians, )
            else:
                gaussians_occupancy = current_occupancy[:, -1]
            occupied_centers_loss = config["occupied_centers_weight"] * (
                        config["sdf_default_isosurface"] - gaussians_occupancy).clamp(min=0.).mean()
        else:
